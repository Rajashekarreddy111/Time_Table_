...
# ---------------------------------------------------------------------------
# Opt 3: Pre-computed consecutive-window masks (built once at import time)
# ---------------------------------------------------------------------------
# Each entry: (bitmask_of_periods, start_period_1indexed)
# e.g. for block_size=3: (0b0000111, 1), (0b0001110, 2), ...
def _consecutive_masks(block_size: int) -> list[tuple[int, int]]:
    result: list[tuple[int, int]] = []
    full_mask = (1 << block_size) - 1  # e.g. block_size=3 -> 0b111
    for start in range(1, 8 - block_size + 1):  # start period 1-indexed
        mask = full_mask << (start - 1)  # shift to period position
        result.append((mask, start))
    return result


_CONSEC_MASKS: dict[int, list[tuple[int, int]]] = {
    bs: _consecutive_masks(bs) for bs in range(1, 8)
}

# Full mask for all 7 periods
_ALL_PERIODS_MASK = 0b1111111  # bits 0-6 = periods 1-7


def _mask_for_periods(periods: list[int]) -> int:
    """Convert a list of 1-indexed period numbers to a bitmask."""
    mask = 0
    for p in periods:
        mask |= (1 << (p - 1))
    return mask
...
def _periods_from_mask(mask: int) -> list[int]:
    """Convert a bitmask to a list of 1-indexed period numbers."""
    return [p for p in range(1, 8) if mask & (1 << (p - 1))]


# ---------------------------------------------------------------------------
# Opt 1 + 2 + 5 + 8: Solver state with bitmask slots and fast counters
# ---------------------------------------------------------------------------
@dataclass
class _SolverState:
    """Mutable solver state using bitmask representations for O(1) slot ops."""
    # section_free[sec_idx][day_idx] = bitmask of free periods (bit i = period i+1)
    section_free: list[list[int]] = field(default_factory=list)
    # faculty_free[fid][day_idx] = bitmask of free periods
    faculty_free: dict[str, list[int]] = field(default_factory=dict)
    # avail_masks[fid][day_idx] = pre-built availability bitmask (Opt 2)
    avail_masks: dict[str, list[int]] = field(default_factory=dict)
    # subject_day_count[sec_idx][day_idx] = {subject_id: count} (Opt 8)
    subject_day_count: list[list[dict[str, int]]] = field(default_factory=list)
    # Incremental remaining hours per requirement index (Opt 5)
    sec_remaining: dict[int, int] = field(default_factory=dict)  # req_idx -> remaining
    # Index maps
    sec_idx: dict[str, int] = field(default_factory=dict)        # section -> index
    day_idx: dict[str, int] = field(default_factory=dict)        # day -> index
    num_sections: int = 0
    num_days: int = 6

    def sec_free_count(self, sec_idx_val: int) -> int:
        """Count of free slots for a section across all days (Opt 5)."""
        return sum(bin(self.section_free[sec_idx_val][di]).count('1')
...
def _build_solver_state(
    ...
    """Build a fresh _SolverState from the current schedule/busy state."""
    state = _SolverState()
    state.sec_idx = {sec: i for i, sec in enumerate(sections)}
    state.day_idx = {day: i for i, day in enumerate(DAYS)}
    state.num_sections = len(sections)
    state.num_days = len(DAYS)

    # Opt 2: pre-build availability masks
    for fid, day_avail in faculty_availability.items():
        masks: list[int] = []
        for day in DAYS:
            avail_periods = day_avail.get(day, set(PERIODS))
            masks.append(_mask_for_periods(list(avail_periods)))
        state.avail_masks[fid] = masks
    # Default availability for faculties not in availability dict
    _default_avail = [_ALL_PERIODS_MASK] * len(DAYS)

    # Opt 1: build section_free bitmasks from current schedule
    state.section_free = []
    for sec in sections:
        day_masks: list[int] = []
        for day in DAYS:
            mask = 0
            for p in PERIODS:
                if schedules[(year, sec)][day][p] is None:
                    mask |= (1 << (p - 1))
            day_masks.append(mask)
        state.section_free.append(day_masks)

    # Opt 1: build faculty_free bitmasks from current busy state
    all_fids: set[str] = set(faculty_busy.keys()) | set(faculty_availability.keys())
    for req in requirements:
        all_fids.update(req.faculty_options or (req.faculty_id,) if req.faculty_id else ())
        all_fids.update(req.faculty_team)
    for fid in all_fids:
        avail_mask_for_fid = state.avail_masks.get(fid, _default_avail)
        busy_set = faculty_busy.get(fid, set())
        day_masks = []
        for di, day in enumerate(DAYS):
            # Start with availability mask, then clear busy periods
            mask = avail_mask_for_fid[di]
            for (bday, bperiod) in busy_set:
                if bday == day and 1 <= bperiod <= 7:
                    mask &= ~(1 << (bperiod - 1))
            day_masks.append(mask)
        state.faculty_free[fid] = day_masks

    # Opt 8: subject_day_count[sec_idx][day_idx] = {subject_id: count}
    state.subject_day_count = [
        [{} for _ in range(len(DAYS))] for _ in range(len(sections))
    ]
    for si, sec in enumerate(sections):
        for di, day in enumerate(DAYS):
            for p in PERIODS:
                entry = schedules[(year, sec)][day][p]
                if entry:
                    sid = str(entry.get("subjectId", "") or entry.get("subject", "")).strip()
                    if sid:
                        dc = state.subject_day_count[si][di]
                        dc[sid] = dc.get(sid, 0) + 1

    # Opt 5: sec_remaining per requirement index
    state.sec_remaining = {}
    return state
...
def _score_slot_candidate(
    ...
    """Legacy scorer kept for compatibility. Hot path uses bitmask scorer below."""
    ...
    conflict_penalty = -5 if faculty_load + block_size > 5 else 0
    ...
    return (
        section_free_score + faculty_available_score + continuous_score
        + conflict_penalty + shared_bonus + longer_block_bonus + center_bonus
        + scarcity_bonus + section_flex + faculty_flex + faculty_load_penalty
    )


def _score_slot_bitmask(
    requirement: "Requirement",
    fid: str,
    day_idx: int,
    start_period: int,
    ...
    """
    Opt 10: Lightweight O(1) candidate scoring using pre-computed bitmask counters.
    No grid scans, no set iterations — only bin().count('1') on 7-bit integers.
    No hard per-day subject cap — only soft clustering preference.
    """
    # Primary: section flexibility (count of free slots for each section today)
    section_flex = 0
    for sec in requirement.sections:
        si = state.sec_idx.get(sec)
        if si is not None:
            section_flex += bin(state.section_free[si][day_idx]).count('1')

    # Faculty flexibility today
    fac_free_mask = state.faculty_free.get(fid, [_ALL_PERIODS_MASK] * len(DAYS))
    faculty_flex = bin(fac_free_mask[day_idx]).count('1')

    # Continuous bonus: prefer blocks that meet the minimum consecutive requirement
    continuous_bonus = 20 if block_size >= requirement.min_consecutive_hours else -15

    # Center bonus: avoid the very first or last period
    center_bonus = 2 if start_period not in (1, 7) else 0

    # Shared bonus
    shared_bonus = 3 if requirement.shared else 0

    # Opt 8: soft clustering penalty (NOT a hard block — just prefer variety per day)
    subject_today = 0
    for sec in requirement.sections:
        si = state.sec_idx.get(sec)
        if si is not None:
            subject_today += state.subject_day_count[si][day_idx].get(requirement.subject_id, 0)
    clustering_penalty = -subject_today  # soft only, no hard cap

    return block_size * 20 + section_flex + faculty_flex + continuous_bonus + center_bonus + shared_bonus + clustering_penalty
...
def _score_slot_candidate_fast(
    ...
    """Compatibility shim — delegates to the legacy scorer."""
    return _score_slot_candidate(
        requirement, faculty_id, day, start_period, block_size,
        schedules, faculty_busy, faculty_availability, year,
    )


def _enumerate_slot_candidates_bitmask(
    requirement: "Requirement",
    remaining_hours: int,
    state: _SolverState,
    schedules: dict[tuple[str, str], dict[str, dict[int, "dict | None"]]],
    ...
    """
    Opt 1+2+3+10: Enumerate valid slot candidates using bitmask operations.
    O(1) slot-free checks via bitmask AND instead of set iteration.
    Opt 3: Window enumeration via pre-computed _CONSEC_MASKS (no runtime arithmetic).
    Opt 10: Scored with lightweight bitmask scorer.
    """
    candidates: list[SlotCandidate] = []
    block_sizes = _candidate_block_sizes(
        remaining_hours, requirement.min_consecutive_hours, requirement.max_consecutive_hours
    )

    # Determine faculty options
    if requirement.faculty_team:
        faculty_options_list: list[tuple[str, tuple[str, ...]]] = [
            (",".join(requirement.faculty_team), requirement.faculty_team)
        ]
    else:
        raw_options = requirement.faculty_options or ((requirement.faculty_id,) if requirement.faculty_id else ())
        faculty_options_list = [(fid, (fid,)) for fid in raw_options]

    for bs in block_sizes:
        block_mask_base = (1 << bs) - 1  # e.g. bs=3 -> 0b111
        consec_windows = _CONSEC_MASKS[bs]  # Opt 3: pre-computed

        for day in days_order:
            di = state.day_idx.get(day, -1)
            if di < 0:
                continue

            # Opt 1: Build shared section mask — intersection of all section free masks
            section_mask = _ALL_PERIODS_MASK
            for sec in requirement.sections:
                si = state.sec_idx.get(sec)
                if si is None:
                    section_mask = 0
                    break
                section_mask &= state.section_free[si][di]
            if not section_mask:
                continue

            for fid_str, fid_tuple in faculty_options_list:
                # Opt 1+2: faculty free mask (already pruned by availability at state build time)
                if requirement.faculty_team:
                    # All team members must be free
                    fac_mask = _ALL_PERIODS_MASK
                    for fid in fid_tuple:
                        fac_masks = state.faculty_free.get(fid, [_ALL_PERIODS_MASK] * len(DAYS))
                        fac_mask &= fac_masks[di]
                    scoring_fid = fid_tuple[0] if fid_tuple else fid_str
                else:
                    fac_masks = state.faculty_free.get(fid_str, [_ALL_PERIODS_MASK] * len(DAYS))
                    fac_mask = fac_masks[di]
                    scoring_fid = fid_str

                # Combined free mask: section AND faculty
                combined = section_mask & fac_mask
                if not combined:
                    continue

                # Opt 3: iterate pre-computed windows
                for window_mask, start_period in consec_windows:
                    if not _is_allowed_compulsory_block_placement(requirement, start_period, bs):
                        continue
                    # Opt 1: O(1) availability check via bitmask AND
                    if (combined & window_mask) != window_mask:
                        continue

                    # Opt 10: fast bitmask scoring
                    score = _score_slot_bitmask(requirement, scoring_fid, di, start_period, bs, state)
                    candidates.append(
                        SlotCandidate(
                            day=day,
...
def _enumerate_slot_candidates(
    ...
    """Legacy fallback enumeration (not used in hot path; kept for compatibility)."""
    candidates: list[SlotCandidate] = []
        ...
        for day in days_order:
            for start_period in periods_order:
                faculty_id = _choose_faculty_for_slot(
                    requirement, day, start_period, block_size, faculty_busy, faculty_availability,
                )
                if not faculty_id:
                    continue
                if not _slot_is_free(
                    requirement.sections, requirement, day, start_period, block_size,
                    schedules, faculty_busy, faculty_availability, year,
                ):
                    continue
                ...
                candidates.append(
                    SlotCandidate(
                        ...
                        score=_score_slot_candidate(
                            requirement, scoring_faculty_id, day, start_period, block_size,
                            schedules, faculty_busy, faculty_availability, year,
                        ),
                    )
                )
...
def _select_next_requirement_mrv(
    requirements: list["Requirement"],
    sec_remaining: dict[int, int],
    state: _SolverState,
    schedules: dict[tuple[str, str], dict[str, dict[int, "dict | None"]]],
    faculty_busy: dict[str, set[tuple[str, int]]],
    faculty_availability: dict[str, dict[str, set[int]]],
    ...
    """
    Opt 6: MRV heuristic — pick requirement with fewest valid placements.
    Immediately returns a 0-candidate requirement to signal early backtrack.
    """
    ...
    for req_idx, req in enumerate(requirements):
        remaining = sec_remaining.get(req_idx, 0)
        if remaining <= 0 or not req.faculty_id:
            continue
        # Opt 6: compute actual valid candidates (the MRV count)
        candidates = _enumerate_slot_candidates_bitmask(
            req, remaining, state, schedules, faculty_busy, faculty_availability,
            year, days_order, candidate_limit,
        )
        key = (
            len(candidates),
            0 if req.shared else 1,
            req.phase,
            -req.hours,
            req.subject_id,
            ",".join(req.sections),
        ...
        if best_key is None or key < best_key:
            ...
            if len(candidates) == 0:
                # Fail-first: 0-candidate requirement detected → prune immediately
                break
...
def _select_next_requirement(
    ...
    """Legacy selector kept for compatibility; hot path uses _select_next_requirement_mrv."""
    best_idx: int | None = None
    best_candidates: list[SlotCandidate] = []
    ...
    for req_idx, requirement in enumerate(requirements):
        ...
        requirement.phase = _determine_requirement_phase(
            requirement.shared, requirement.min_consecutive_hours, requirement.hours,
            requirement.faculty_options, faculty_availability,
        )
    ...
    for req_idx, requirement in enumerate(requirements):
        ...
        candidates = _enumerate_slot_candidates(
            requirement, remaining, schedules, faculty_busy, faculty_availability,
            year, days_order, periods_order, candidate_limit,
        )
        key = (
            len(candidates),
            0 if requirement.common_faculty else 1,
...
def _place_block(
    ...
    # Opt 9: compute block mask once for vectorised updates
    block_mask = _mask_for_periods(periods)
    ...
    # Opt 1: update bitmask state if provided
    if state is not None:
        di = state.day_idx.get(day, -1)
        if di >= 0:
            # Opt 1: clear placed periods from section_free and faculty_free
            for sec in requirement.sections:
                si = state.sec_idx.get(sec)
                if si is not None:
                    state.section_free[si][di] &= ~block_mask
                    # Opt 8: update subject_day_count
                    dc = state.subject_day_count[si][di]
                    dc[subject_id] = dc.get(subject_id, 0) + block_size
            for fid in faculty_ids:
                if fid in state.faculty_free:
                    state.faculty_free[fid][di] &= ~block_mask
...
def _undo_block(
    ...
    # Opt 1: compute undo mask once
    periods_list = [p for _, p in placements]
    undo_mask = _mask_for_periods(periods_list) if periods_list else 0
    ...
    # Opt 1: restore bitmask state if provided
    if state is not None and undo_mask and placements:
        day = placements[0][0]
        di = state.day_idx.get(day, -1)
        if di >= 0:
            for sec in requirement.sections:
                si = state.sec_idx.get(sec)
                if si is not None:
                    # Restore free slots — intersect with availability mask
                    avail = state.avail_masks.get(
                        next(iter(faculty_ids), ""), [_ALL_PERIODS_MASK] * len(DAYS)
                    )
                    state.section_free[si][di] |= undo_mask
                    # Opt 8: decrease subject_day_count
                    if subject_id:
                        dc = state.subject_day_count[si][di]
                        new_count = dc.get(subject_id, 0) - (block_size or len(periods_list))
                        if new_count <= 0:
                            dc.pop(subject_id, None)
                        else:
                            dc[subject_id] = new_count
            for fid in faculty_ids:
                if fid in state.faculty_free:
                    # Restore free slots — intersect with availability mask so we don't
                    # grant availability the faculty never had.
                    avail_mask_fid = state.avail_masks.get(fid, [_ALL_PERIODS_MASK] * len(DAYS))
                    state.faculty_free[fid][di] |= (undo_mask & avail_mask_fid[di])
...
def generate_timetable(
    ...
    def _solve_with_orders(
        ...
        remaining_by_req.update({req_idx: req.hours for req_idx, req in enumerate(requirements)})

        # Opt 4: Greedy first-pass — fills the easy slots without any backtracking
        def greedy_pass() -> None:
            changed = True
            while changed:
                changed = False
                for req_idx, req in enumerate(requirements):
                    rem = remaining_by_req.get(req_idx, 0)
                    if rem <= 0 or not req.faculty_id:
                        continue
                    cands = _enumerate_slot_candidates_bitmask(
                        req, rem, state, schedules, faculty_busy, faculty_availability,
                        year, days_order, 1,
                    )
                    if not cands:
                        continue
                    best = cands[0]
                    place_source = "shared_class_file" if req.shared else "solver"
                    _place_block(
                        req, best.faculty_ids, best.day, best.start_period, best.block_size,
                        schedules, faculty_busy, year, subject_id_to_name, faculty_id_to_name,
                        session_log, source=place_source, state=state,
                    )
                    remaining_by_req[req_idx] -= best.block_size
                    state.sec_remaining[req_idx] = remaining_by_req[req_idx]
                    changed = True

        greedy_pass()

        # Opt 5: Incremental feasibility using bitmask free-count
        def _is_feasible() -> bool:
            for req_idx, req in enumerate(requirements):
                needed = remaining_by_req.get(req_idx, 0)
                if needed <= 0:
                    continue
                for sec in req.sections:
                    si = state.sec_idx.get(sec)
                    if si is not None and needed > state.sec_free_count(si):
                        return False
            return True

        # Opt 6: Backtrack with MRV heuristic
        def backtrack() -> bool:
            # Opt 5: forward-checking via incremental bitmask counts
            if not _is_feasible():
                return False
            # Opt 6: MRV — pick requirement with fewest valid placements
            next_req_idx, candidates = _select_next_requirement_mrv(
                requirements, remaining_by_req, state, schedules, faculty_busy,
                faculty_availability, year, days_order, candidate_limit,
            )
            ...
            for candidate in candidates:
                ...
                state.sec_remaining[next_req_idx] = remaining_by_req[next_req_idx]
                if backtrack():
                    return True
                remaining_by_req[next_req_idx] += candidate.block_size
                state.sec_remaining[next_req_idx] = remaining_by_req[next_req_idx]
                _undo_block(
                    requirement, candidate.faculty_ids, placements, schedules, faculty_busy,
                    year, session_log, state=state,
                    subject_id=requirement.subject_id, block_size=candidate.block_size,
                )
            return False
    ...
    # Continual search until valid timetable is found
    while not solved:
        ...
        candidate_limit = min(64, 16 + attempt * 8)
        state = _build_solver_state(all_sections, schedules, faculty_busy, faculty_availability, requirements, year)
        
        if _solve_with_orders(days_order, periods_order, candidate_limit, state):
            solved = True
            break
        attempt += 1
